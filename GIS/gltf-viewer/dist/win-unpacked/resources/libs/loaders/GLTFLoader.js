 ...
 */
	...
	_convertObjectToArrays() {
			const keys = [ 'accessors', 'bufferViews', 'buffers', 'cameras', 'images', 'materials', 'meshes', 'nodes', 'samplers', 'scenes', 'skins', 'textures', 'animations' ];
			for ( const key of keys ) {
				if ( this.json[ key ] && typeof this.json[ key ] === 'object' && ! Array.isArray( this.json[ key ] ) ) {
					const obj = this.json[ key ];
					const arr = [];
					const keysArray = Object.keys( obj );
					for ( let i = 0; i < keysArray.length; i ++ ) {
						arr[ i ] = obj[ keysArray[ i ] ];
					}
					this.json[ key ] = arr;
				}
			}

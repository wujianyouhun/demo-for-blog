...
# 设置torch.load的兼容性模式
torch.load.__defaults__ = (False,)  # 设置weights_only=False为默认值
...
class FruitDetector:
    ...
    def _load_models(self):
        ...
        try:
            ...
            # 尝试加载大模型，如果失败则使用小模型
            try:
                self.detector_model = YOLO('yolov8x.pt')
                self.seg_model = YOLO('yolov8x-seg.pt')
                print("Large models (yolov8x) loaded successfully!")
            except Exception as e:
                print(f"Failed to load large models: {e}")
                print("Loading small models (yolov8n)...")
                self.detector_model = YOLO('yolov8n.pt')
                self.seg_model = YOLO('yolov8n-seg.pt')
                print("Small models loaded successfully!")
    ...
    def detect_fruits(self, image_path, save_prefix):
        ...
        try:
            # 使用检测模型进行目标检测
            detection_results = self.detector_model(img_rgb, device=self.device, conf=0.5, verbose=False)

            # 使用分割模型进行实例分割
            seg_results = self.seg_model(img_rgb, device=self.device, conf=0.5, verbose=False)
        except Exception as e:
        ...
        # 保存结果图片
        detection_result_path = os.path.join(self.result_dir, f"{save_prefix}_yolov8_detection.jpg")
        seg_result_path = os.path.join(self.result_dir, f"{save_prefix}_yolov8_seg.jpg")
    ...
    def save_results_to_text(self, results):
        ...
        with open(text_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"检测模型: YOLOv8 (目标检测) + YOLOv8-seg (实例分割)\n")

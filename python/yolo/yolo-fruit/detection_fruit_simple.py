...
# 解决PyTorch 2.6+的兼容性问题
torch.serialization.add_safe_globals([ultralytics.nn.tasks.DetectionModel, ultralytics.nn.tasks.SegmentationModel])

class SimpleFruitDetector:
    def __init__(self):
        # 初始化设备
        self.device = 'cuda' if hasattr(YOLO, 'device') and 'cuda' in str(YOLO.device) else 'cpu'
    ...
    def _load_models(self):
        ...
        try:
            ...
            # 使用预训练的模型，让ultralytics自动处理下载和加载
            self.detector_model = YOLO('yolov8n.pt')  # 使用小模型确保兼容性
...
def main():
    ...
    try:
        ...
        print("Please check:")
        print("1. Required libraries installed (ultralytics, opencv-python)")
